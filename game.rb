...
while @player_moves.size <= 4
	...
	# Проверка, чтобы юзер не ввел ерунду
        while !all_moves.include?(move_player) do
    ...
    # Варианты при которых юзер выигрывает
        ...
        if @comp_moves.size != 4
           ...
           comp_move = rand(1..9)
            ...
            while @comp_moves.include?(comp_move.to_s) &&
        	@player_moves.include?(comp_move.to_s) do
            puts 'Ход компьютера.'
            comp_move = rand(1..9)
        	end
...
end	# Финал метода player_vs_comp
		
		def comp_vs_player # Компьютер против юзера
			puts "\nКомпьютер ходит.\n"
			sleep 2
		end # Финал метода comp_vs_player
